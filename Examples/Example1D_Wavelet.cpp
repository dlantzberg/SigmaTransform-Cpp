...
int main( int argc, char** argv ) {
    try {
        ...
        // analyze
        Wavelet1D.analyze( bat_signal );
        Chrono.toc("analyze").tic();

        // save windows
        // sigma::save2file_bin("bat_windows.bin",Wavelet1D.getWindows() );
        // Chrono.toc("saveWindows").tic();

        // save coefficients
        // sigma::save2file_bin("bat_coeff.bin",Wavelet1D.getCoeffs() );
        // Chrono.toc("saveCoeffs").tic();

        // mask coefficients
        Wavelet1D.applyMask( [&]( sigma::point<1>const& x, sigma::point<1>const& step )->sigma::cmpx {
            return ( x > .0005 && x < .002 ) && (step < 16 && step > 12);
        } );
        Chrono.toc("mask").tic();

        // save masked coefficients
        // sigma::save2file_bin("bat_coeff_masked.bin",Wavelet1D.getCoeffs() );
        // Chrono.toc("saveMaskedCoeffs").tic();

        // synthesize
        Wavelet1D.synthesize();
        Chrono.toc("synthesize").tic();

        // save reconstruction
        // sigma::save2file_bin("bat_rec.bin",Wavelet1D.getReconstruction() );
